...
const CategoryList = () => {
  ...
  useEffect(() => {
    const getCategories = async () => {
      try{
        ...
        //api request
        const response = await axios.get(`/category?page=${currentPage}&${searchValue}`);
  ...
  const handleDelete = async () => {
    try{
      ...
      const response2 = await axios.get(`/category?page=${currentPage}&${searchValue}`);
    ...
    <div>
        ...
        <tbody>
          {categories.map((category) => (
            ...
            <th>
              ...
              <button className="button"onClick={() => {setShowModal(true); setCategoryId(category._id)}}>Delete</button>
            </th>
          </tr>
          ))}
      ...
      {pageCount.length > 0 && (
        <div className="pag-container">
          ...
          <button className="pag-button" key={index} onClick={() => handlePage(pageNumber)}>{pageNumber}</button>
          ))}
