const Home = () => {
  return (
    <div>
      <h2 className="table-title">Post list</h2>

      <input
        className="saerch-input"
        type="text"
        name="search"
        placeholder="Search here"
      />

      <div className="flexbox-container wrap">
        <div className="post-card">
          <h4 className="card-title">Post 1</h4>
          <p className="card-desc">
            Lorem, ipsum dolor sit amet consectetur adipisicing elit. Ipsam et
            hic quae fugit sint architecto, libero aperiam ut tempore
            voluptatum.
          </p>
        </div>
        <div className="post-card">
          <h4 className="card-title">Post 2</h4>
          <p className="card-desc">
            Lorem, ipsum dolor sit amet consectetur adipisicing elit. Ipsam et
            hic quae fugit sint architecto, libero aperiam ut tempore
            voluptatum.
          </p>
        </div>
        <div className="post-card">
          <h4 className="card-title">Post 3</h4>
          <p className="card-desc">
            Lorem, ipsum dolor sit amet consectetur adipisicing elit. Ipsam et
            hic quae fugit sint architecto, libero aperiam ut tempore
            voluptatum.
          </p>
        </div>
        <div className="post-card">
          <h4 className="card-title">Post 4</h4>
          <p className="card-desc">
            Lorem, ipsum dolor sit amet consectetur adipisicing elit. Ipsam et
            hic quae fugit sint architecto, libero aperiam ut tempore
            voluptatum.
          </p>
        </div>
        <div className="post-card">
          <h4 className="card-title">Post 5</h4>
          <p className="card-desc">
            Lorem, ipsum dolor sit amet consectetur adipisicing elit. Ipsam et
            hic quae fugit sint architecto, libero aperiam ut tempore
            voluptatum.
          </p>
        </div>
        <div className="post-card">
          <h4 className="card-title">Post 6</h4>
          <p className="card-desc">
            Lorem, ipsum dolor sit amet consectetur adipisicing elit. Ipsam et
            hic quae fugit sint architecto, libero aperiam ut tempore
            voluptatum.
          </p>
        </div>
        <div className="post-card">
          <h4 className="card-title">Post 7</h4>
          <p className="card-desc">
            Lorem, ipsum dolor sit amet consectetur adipisicing elit. Ipsam et
            hic quae fugit sint architecto, libero aperiam ut tempore
            voluptatum.
          </p>
        </div>
        <div className="post-card">
          <h4 className="card-title">Post 8</h4>
          <p className="card-desc">
            Lorem, ipsum dolor sit amet consectetur adipisicing elit. Ipsam et
            hic quae fugit sint architecto, libero aperiam ut tempore
            voluptatum.
          </p>
        </div>
        <div className="post-card">
          <h4 className="card-title">Post 1</h4>
          <p className="card-desc">
            Lorem, ipsum dolor sit amet consectetur adipisicing elit. Ipsam et
            hic quae fugit sint architecto, libero aperiam ut tempore
            voluptatum.
          </p>
        </div>
        <div className="post-card">
          <h4 className="card-title">Post 1</h4>
          <p className="card-desc">
            Lorem, ipsum dolor sit amet consectetur adipisicing elit. Ipsam et
            hic quae fugit sint architecto, libero aperiam ut tempore
            voluptatum.
          </p>
        </div>
      </div>

      <div className="pag-container">
        <button className="pag-button">prev</button>
        <button className="pag-button">1</button>
        <button className="pag-button">2</button>
        <button className="pag-button">3</button>
        <button className="pag-button">next</button>
      </div>
    </div>
  );
};

export default Home;
